...
use speedysearch::index::{
    index_applications_in, index_settings, EntryType, IndexEntry, SearchIndex,
};
...
pub fn warm_file_index(state: LauncherState) {
    ...
    if let Err(error) = state
        .launcher
        .index
        .discover_files(&state.roots, &state.excludes)
...
pub async fn log_click(
    ...
    let selected_id = selected_id
        .parse::<u64>()
        .map_err(|_| "invalid result id".to_string())?;
    tauri::async_runtime::spawn_blocking(move || {
        launcher.log_selection(&query, selected_id, rank_position)
    })
    .await
    .map_err(|error| error.to_string())?
    .map_err(|error| error.to_string())
...
pub async fn show_in_folder(state: State<'_, LauncherState>, id: String) -> Result<(), String> {
    ...
    if entry.entry_type != EntryType::File {
        return Err("Only files and folders have a containing folder".into());
    }
    let path = Path::new(&entry.path);
    let folder = if entry.metadata.is_dir {
        path
    } else {
...
pub fn set_global_hotkey(
    ...
    if let Err(error) = write_ui_config(&config) {
        match &state.hotkey_backend {
            ...
            HotkeyBackend::Cosmic { custom_path, .. } => {
                let _ = write_cosmic_shortcut(custom_path, None, None);
            }
...
fn find_entry(launcher: &SearchLauncher, id: u64) -> Result<IndexEntry, String> {
    launcher
        ...
        .cloned()
        .ok_or_else(|| format!("result {id} is no longer indexed"))
}
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        if wayland {
            ...
            if desktop.split(':').any(|part| part == "cosmic") {
                ...
                let executable =
                    std::env::current_exe().unwrap_or_else(|_| PathBuf::from("speedysearch-ui"));
                return HotkeyBackend::Cosmic {
                    custom_path: config_root
                        .join("cosmic/com.system76.CosmicSettings.Shortcuts/v1/custom"),
                    defaults_path: PathBuf::from(
...
fn write_cosmic_shortcut(
    ...
    if let Some((shortcut, command)) = registration {
        ...
        let compact_defaults = defaults_path
            .and_then(|path| std::fs::read_to_string(path).ok())
            .map(|value| compact_ron(&value))
            .unwrap_or_default();
    ...
    if let Some(parent) = custom_path.parent() {
        std::fs::create_dir_all(parent)
            .map_err(|error| format!("could not create COSMIC shortcut directory: {error}"))?;
    }
    let temporary = custom_path.with_extension("tmp");
    std::fs::write(&temporary, contents)
        .map_err(|error| format!("could not write COSMIC shortcut: {error}"))?;
    std::fs::rename(&temporary, custom_path)
        .map_err(|error| format!("could not activate COSMIC shortcut: {error}"))
}
...
fn cosmic_shortcut_entry(shortcut: &str, command: &str) -> Result<(String, String), String> {
    ...
    let modifiers: Vec<&str> = ["Super", "Ctrl", "Alt", "Shift"]
        .into_iter()
        .enumerate()
        .filter_map(|(index, modifier)| modifier_flags[index].then_some(modifier))
        .collect();
...
fn remove_managed_cosmic_shortcut(contents: &str) -> Result<String, String> {
    let open = contents
        .find('{')
        .ok_or_else(|| "COSMIC shortcut config is not a map".to_string())?;
    let close = contents
        .rfind('}')
    ...
    for (index, character) in inner.char_indices() {
        ...
        match character {
            ...
            ',' if parens == 0 && brackets == 0 && braces == 0 => {
                ...
                if !is_managed_cosmic_shortcut(segment) {
    ...
    if !is_managed_cosmic_shortcut(tail) {
        kept.push_str(tail);
    }
    Ok(format!(
        "{}{}{}",
...
mod tests {
    ...
    fn global_hotkeys_require_a_real_modifier_and_key() {
        assert_eq!(
            normalize_hotkey("Ctrl + Alt + K").as_deref(),
            Ok("Ctrl+Alt+K")
        );
    ...
    fn cosmic_shortcut_writer_is_atomic_and_detects_conflicts() -> anyhow::Result<()> {
        ...
        write_cosmic_shortcut(
            &custom,
            Some(&defaults),
            Some(("Ctrl+Space", "/usr/bin/speedysearch-ui")),
        )
        ...
        assert!(write_cosmic_shortcut(
            &custom,
            Some(&defaults),
            Some(("Super+Space", "/usr/bin/speedysearch-ui"))
        )
        .is_err());
